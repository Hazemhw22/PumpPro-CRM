...
const Header = () => {
    ...
    useEffect(() => {
        const fetchProfile = async () => {
            if (user?.user?.id) {
                const { data, error } = await supabase.from('profiles').select('avatar_url').eq('id', user.user.id).single<Pick<Profile, 'avatar_url'>>();
                if (data) {
                ...
                <div className="relative flex w-full items-center bg-white px-5 py-2.5 dark:bg-black">
                    ...
                    <div className="hidden ltr:mr-2 rtl:ml-2 sm:block">
                        <ul className="flex items-center space-x-2 rtl:space-x-reverse dark:text-[#d0d2d6]">
                            <li>
                                <Link href="/(defaults)/bookings/add" className="block rounded-full bg-white-light/40 p-2 hover:bg-white-light/90 hover:text-primary dark:bg-dark/40 dark:hover:bg-dark/60">
                                    <IconEdit />
                                </Link>
                            </li>
                        </ul>
