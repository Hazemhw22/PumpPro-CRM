...
const CustomerSelect = ({ selectedCustomer, onCustomerSelect, onCreateNew, className = '' }: CustomerSelectProps) => {
    ...
    const fetchCustomers = async () => {
        ...
        try {
            const { data, error } = await supabase
                .from('customers')
                .select('id, name, type, email, phone, address, business_name, tax_id')
                .order('name');
                                    ...
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <div className="font-medium text-black dark:text-white truncate">
                                                {customer.name}
                                                {customer.business_name && <span className="text-xs text-gray-500 ml-2">({customer.business_name})</span>}
                                            </div>
                                            <div className="text-sm text-gray-500 dark:text-gray-400">{customer.phone}</div>
                                            <div className="text-xs text-gray-400 dark:text-gray-500">
                                                {customer.type === 'business' ? t('business') || 'Business' : t('private') || 'Private'}
