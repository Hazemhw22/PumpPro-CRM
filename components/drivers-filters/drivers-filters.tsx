...
interface Props {
    onFilterChange: (f: DriversFilters) => void;
    onClearFilters: () => void;
    viewMode?: 'list' | 'grid';
    onViewModeChange?: (mode: 'list' | 'grid') => void;
}

const DriversFilters: React.FC<Props> = ({ onFilterChange, onClearFilters, viewMode = 'list', onViewModeChange }) => {
    const { t } = getTranslation();
    const [isOpen, setIsOpen] = useState(false);
    const [filters, setFilters] = useState<DriversFilters>({ search: '', status: '', licenseFrom: '', licenseTo: '' });
    ...
    const clear = () => {
        setFilters({ search: '', status: '', licenseFrom: '', licenseTo: '' });
        onClearFilters();
    };
            ...
            {isOpen && (
                <div className="panel border-white-light px-5 py-4 dark:border-[#1b2e4b]">
                    <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-6 xl:grid-cols-4">
                        ...
                        <div className="space-y-2">
                            <label className="text-sm font-medium text-gray-700 dark:text-gray-300">{t('license_expiry_from')}</label>
                            <input type="date" className="form-input" value={filters.licenseFrom} onChange={(e) => setFilters((p) => ({ ...p, licenseFrom: e.target.value }))} />
                        </div>

                        <div className="space-y-2">
                            <label className="text-sm font-medium text-gray-700 dark:text-gray-300">{t('license_expiry_to')}</label>
                            <input type="date" className="form-input" value={filters.licenseTo} onChange={(e) => setFilters((p) => ({ ...p, licenseTo: e.target.value }))} />
                        </div>
                    </div>
                </div>
            )}
